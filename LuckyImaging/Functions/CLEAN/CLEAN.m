function [ clean] = CLEAN( image, config ,i )
    ...
    clean.indexCheck{1,1} = [0 , 0]; %initilize index check vector
    ...
    while threshold>=config.cleanThreshold
            ...
            clean.fitGaussian=FitGaussian( config , clean.brightFinder.matrix, count); % step 2
             ...
             clean.generateGaussianMask=GenerateGaussianMask(config, clean , clean.indexCheck{1,count}); % step 3
             image=image-clean.generateGaussianMask.mask;                                                    % step 3
                
                
            if clean.indexCheck{1,count}(1)==clean.brightFinder.rowIndex && clean.indexCheck{1,count}(2)==clean.brightFinder.colIndex
                ...
                if  clean.fitGaussian.exitflag ==0    % if the fit has failed don't update outputArray, imageGaussian.
                    ...
                    clean.outputArray{1,count}=clean.fitGaussian.fit;                     % update the output values
                    clean.outputArray{2,count}=clean.fitGaussian.resnorm;
                    clean.outputArray{1,count}(1)=clean.outputArray{1,count}(1)+clean.brightFinder.colIndex-(config.brightFinderSize+1);
                    clean.outputArray{1,count}(2)=clean.outputArray{1,count}(2)+clean.brightFinder.rowIndex-(config.brightFinderSize+1);
                    ...
                    clean.indexCheck{1,count}=[clean.brightFinder.rowIndex , clean.brightFinder.colIndex];   %update the indexChecker for the mask generation
